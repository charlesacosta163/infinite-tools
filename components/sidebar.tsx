  ...
  return (
    <aside className='w-[225px] py-8 px-4 flex flex-col justify-between h-full'>
            ...
            <div className="grid grid-cols-2 gap-2">
              {
                links.map((link, index) => (
                  <Link key={index} href={link.path} className={`flex flex-col px-2 py-5 items-center rounded-md hover:bg-accentOriginal dark:hover:bg-gray-800 hover:text-white text-sm group cursor-pointer text-[#966554] dark:text-white tracking-tight transition-all duration-200 ${isActive(link.path) ? "bg-accentOriginal dark:bg-gray-800 text-white [&>span]:text-white" : ""} ${index === links.length - 1 ? "col-span-2" : ""}`}>
                    <span className='group-hover:text-white text-accentOriginal dark:text-gray-400 text-lg flex items-center justify-center rounded-md'>{link.icon}</span>
                    {link.name}
                  </Link>
                ))
              }
      ...
      <div className="mt-2 bg-gradient-to-br from-gray-800 to-gray-900 py-4 px-2 rounded-xl text-white flex flex-col items-center justify-between gap-3 relative overflow-hidden">
         ...
         <p className='text-xl text-center tracking-tighter relative z-10 font-black'>Infinite Flight Community</p>
         <Link 
         target='_blank'
